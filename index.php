...
LHC_API.args = {mode:'widget',lhc_base_url:'//chatqna.bpsaceh.com/index.php/',wheight:450,wwidth:350,pheight:520,pwidth:500,leaveamessage:true,check_messages:false};
(function() {
var po = document.createElement('script'); po.type = 'text/javascript'; po.setAttribute('crossorigin','anonymous'); po.async = true;
var date = new Date();po.src = '//chatqna.bpsaceh.com/design/defaulttheme/js/widgetv2/index.js?'+(""+date.getFullYear() + date.getMonth() + date.getDate());
...
function print_add_ticket()
{
    ...
    foreach ($hesk_settings['custom_fields'] as $k=>$v) {
        if ($v['use'] == 1 && hesk_is_custom_field_in_category($k, $category)) {
            if ($v['type'] == 'checkbox') {
                $k_value = array();
                if (isset($SESSION["c$k"]) && is_array($SESSION["c$k"])) {
                    foreach ($SESSION["c$k"] as $myCB) {
                        $k_value[] = stripslashes(hesk_input($myCB));
                    }
                }
            } elseif (isset($SESSION["c$k"])) {
                $k_value = stripslashes(hesk_input($SESSION["c$k"]));
            } else {
                $k_value = '';
            }
            ...
            switch ($v['type']) {
                ...
                // Hidden
                case 'hidden':
                    if (strlen($k_value) != 0 || isset($SESSION["c$k"])) {
...
function print_start()
{
    ...
    if ($hesk_settings['kb_enable'])
    {
        ...
        $latest_articles = hesk_kbLatestArticles($hesk_settings['kb_latest'], 0);
    }
